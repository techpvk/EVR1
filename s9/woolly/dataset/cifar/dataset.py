...
from woolly.utils.transform import convert_to_tensor
...
class WyCustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Enable this when using Albementation lib for transform
...
class WyDatasetAdvance(Dataset):
    ...
    def ricap(self, ricap_beta=0.3):
        ...
        for k in range(4):
            ...
            start_y = np.random.randint(0, image_height - cropped_height[k] + 1)
            ...
            cropped_images[k] = image[
                start_x : start_x + cropped_width[k],
                start_y : start_y + cropped_height[k],
                :,
            ]
            labels[k] = self.dataset[idx][1]
            weightage[k] = (
                cropped_width[k] * cropped_height[k] / (image_width * image_height)
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.apply_ricap and self.apply_cutout:
            ...
            if np.random.rand() < recap_prob:
                image, label, weightage, applied_ricap = self.ricap(
...
def get_cifar_loader(train_transform, test_transform, batch_size=64, use_cuda=True):
    """Get an instance of train and test loaders for CIFAR10 data
    ...
    Args:
        ...
        batch_size (int, optional): batch size to be used in training. It defaults to 64.
        use_cuda (bool, optional): Enable/Disable Cuda GPU. Defaults to True.

    Returns:
        (DataLoader, DataLoader): Get an instance of train and test data loaders
    """
...
def get_advance_cifar_loader(
    ...
    test_loader = DataLoader(
        WyDatasetAdvance(
            datasets.CIFAR10("../data", train=False, download=True), normalize=normalize
        ),
        batch_size=batch_size,
        shuffle=True,
        **kwargs
