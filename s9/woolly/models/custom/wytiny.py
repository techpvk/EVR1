...
class WyBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = WyConv2d(input_size, planes, kernel_size=1, padding=0, strides=1)
        ...
        self.conv3 = WyConv2d(planes, output_size, kernel_size=1, padding=0, strides=1)
...
class WyTiny(nn.Module):
    ...
    def __init__(
        ...
        for i, s in [(2, 1), (2, 1), (2, 1)]:  # range(self.blocks_count):
            ...
            self.blocks.append(
                self._make_block(usedilation, i, s, resblock=self.resblock)
            )
            self.base_channels = self.base_channels * self.resblock.expansion
    ...
    def _make_block(self, usedilation, repetations, strides, resblock):
        ...
        if strides != 1:
            ...
            inc, outc = (
                self.base_channels * resblock.expansion,
                self.base_channels * resblock.expansion,
            )
        block = WyBlock(
            inc,
