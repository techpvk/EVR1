...
def train_native(use_l1=False, lambda_l1=5e-4):
    ...
    def internal(
        ...
        for data, target, weightage, ricap in train_loader:
            data, target, weightage, ricap = (
                data.to(device),
                [t.to(device) for t in target],
                [w.to(device) for w in weightage],
                ricap.to(device),
            )
            ...
            for k in range(4):
                loss = (
                    loss
                    + (
                        weightage[k][true_indices]
                        * criteria(
                            output[true_indices],
                            target[k][true_indices],
                            reduction="none",
                        )
                    ).mean()
                )
                ...
                batch_correct += (
                    (
                        weightage[k][true_indices]
                        * pred.eq(target[k][true_indices].view_as(pred)).float().t()
                    )
                    .sum()
                    .item()
                )
...
def train_ricap(use_l1=False, lambda_l1=5e-4, ricap_beta=0.3):
    ...
    def internal(
        ...
        def ricap(data, target):
            ...
            eploss = sum(
                [W_[k] * criteria(output[0:batch_size], c_[k]) for k in range(4)]
            )
            acc = sum(
                [W_[k] * accuracy(output[0:batch_size], c_[k])[0] for k in range(4)]
            )
            ...
            acc = acc.item() + (
                100 * pred.eq(target.view_as(pred)).sum().item() / batch_size
            )
    ...
    def internal(
        ...
        def ricap(data, target):
            ...
            for k in range(4):
                ...
                c_ += torch.sparse_coo_tensor(
                    torch.cat((seq, indices), dim=0), weight, (batch_size, classes)
                ).to_dense()
            ...
            target = torch.cat((c_, F.one_hot(target, num_classes=classes)), dim=0)
        ...
        for data, target in train_loader:
            ...
            with autocast():
                ...
                pred = output.argmax(dim=1, keepdim=True)
                batch_correct = (
                    100 * pred.eq(target.view_as(pred)).sum().item() / target.size[0]
                )
