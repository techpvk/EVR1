...
def layout(
    ...
    **kw
    ...
    axs = [
        figure(ax, **kw)
        for row in zip(np.array(axs).reshape(nrows, ncols), figures)
        for ax, figure in zip(*row)
    ]
...
def train_test_load(batch_size, use_cuda, ricap_beta):
    ...
    train_set = preprocess(
        dataset["train"],
        [partial(pad, border=4), transpose, normalise, to(torch.float16)],
    )
    valid_set = preprocess(dataset["valid"], [transpose, normalise, to(torch.float16)])

    train_batches = partial(
        Batches, dataset=train_set, shuffle=True, drop_last=False, max_options=200
    )
    valid_batches = partial(Batches, dataset=valid_set, shuffle=False, drop_last=False)

    return train_batches(
        batch_size=batch_size, transforms=(Crop(32, 32), FlipLR(), Cutout(8, 8))
    ), valid_batches(batch_size=batch_size)
